...
import { clearErrors, newService } from '../actions/ServicesAction';
import { ALERT_TYPE, Dialog } from "react-native-alert-notification"
import { ADD_NEW_SERVICES_RESET } from '../constants/ServiceConstants';
...
const PAddServices = () => {
    ...
    const { error, loading, isPosted } = useSelector((state) => state.service)
    ...
    const Nails = ['Manicure & Padicure', 'Manicure & Padicure (Classic)', 'Manicure & Padicure (Advance)', 'Manicure & Padicure (Luxury)', 'Manicure & Padicure (Extra)', 'Extensions', 'Extensions (Acrylic Extensions)', 'Extensions (Natural Gel-Extensions)', 'Extensions (Polygon gum gel natural)', 'Refills, Tips, Gelpolished & Nails']
    const manicure = ['Advance', 'Luxury', 'Extra', 'Classic'];
    ...
    const handleService = () => {
        dispatch(newService(servicetype, category, servicename, hour, price, about, myemployees))
    }
    ...
    useEffect(() => {
        ...
        if (isPosted) {
            Dialog.show({
                ...
                title: 'Success',
                textBody: "Service Added",
                button: 'close'
            })
            dispatch({
                type: ADD_NEW_SERVICES_RESET
            })
                ...
                <View style={{ marginHorizontal: 20 }}>
                    ...
                    {/* {
                        servicetype === 'Nails' && category === 'Manicure & Padicure' ? <SelectDropdown
                            ...
                            dropdownStyle={styles.dropdown1DropdownStyle}
                            dropdownIconPosition={'right'}
                            defaultButtonText='Choose Sub Category'
                            buttonTextAfterSelection={(selectedItem, index) => {
                                return selectedItem
                            }}
                            rowTextForSelection={(item, index) => {
            ...
            <View>
                {
                    loading ? <TouchableOpacity style={{ marginVertical: 0, backgroundColor: '#232323', paddingHorizontal: 40, paddingVertical: 20 }}>
                        ...
                        <TouchableOpacity onPress={handleService} style={{ marginVertical: 0, backgroundColor: '#232323', paddingHorizontal: 40, paddingVertical: 20 }}>
                            <Text style={{ color: 'white', fontFamily: "Montserrat_700Bold", alignSelf: 'center', fontSize: 16 }}>ADD SERVICE</Text>
                        </TouchableOpacity>
                }
            </View>
        </>
