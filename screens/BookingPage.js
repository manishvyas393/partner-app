...
import { clearErrors, newBookings, newSlot } from '../actions/BookingAction';
import { ALERT_TYPE, Dialog } from 'react-native-alert-notification';
import { BOOK_NEW_APPOINTMENT_RESET } from '../constants/BookingConstants';
...
const BookingPage = ({ route, navigation }) => {
    ...
    const { time, outtym, ids, day, bookedTime, customerBooked, employeeTime, name, phone, inti, outi, timestamp, serviceId, serviceid } = route.params;
    ...
    useEffect(() => {
        ...
        if (employees) {
            ...
            setAbout(employees.about)
        }
        else {
            dispatch(getEmployeesServices(id))
            dispatch(getSingleEmployee(id))
        }
    }, [dispatch, intervals, slotError, error, isPosted, serviceId, id])
                    ...
                    <>
                        <ScrollView style={{ flex: 1, backgroundColor: 'white', }}>
                            ...
                            <View style={{ marginVertical: 10, marginHorizontal: 10 }}>
                                ...
                                {
                                    !about ? null : <Text style={{ fontFamily: 'Montserrat_600SemiBold', fontSize: 12, flex: 5, lineHeight: 20 }}>Note: {about}</Text>
                                }
                            </View>
                        </ScrollView>
